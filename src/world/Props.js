import { BoxGeometry, CatmullRomCurve3, CylinderGeometry, Group, IcosahedronGeometry, InstancedMesh, Matrix4, Mesh, Quaternion, TorusGeometry, TubeGeometry, Vector3 } from 'three';
...
export function createCrate(materials, size = 1) {
  ...
  const bandGeometry = new BoxGeometry(size * .08, size * .86, size * 1.03);
  for (const x of [-size * .28, size * .28]) bands.add(mesh(bandGeometry, materials.metalDark));
  bands.position.y = size * .41;
  group.add(bands);
  const plankGeometry = new BoxGeometry(size * 1.01, size * .035, size * .035);
...
export function createInstancedCrates(materials, count, placements) {
  const geometry = new BoxGeometry(1.15, .9, 1.15);
  const instance = new InstancedMesh(geometry, materials.wood, count);
  instance.castShadow = true;
  instance.receiveShadow = true;
  const matrix = new Matrix4();
  for (let i = 0; i < count; i += 1) {
    const placement = placements[i];
    matrix.compose(new Vector3(placement.x, placement.y ?? .45, placement.z), new Quaternion(), new Vector3(placement.scale ?? 1, placement.scale ?? 1, placement.scale ?? 1));
...
export function createBarrel(materials, color = 'militaryDark') {
  const group = new Group();
  const body = mesh(new CylinderGeometry(.42, .46, 1.05, 20), materials[color] ?? materials.militaryDark);
  body.position.y = .53;
  group.add(body);
  const rimGeometry = new TorusGeometry(.43, .035, 6, 14);
  ...
  const cap = mesh(new CylinderGeometry(.25, .25, .02, 12), materials.warning);
  cap.position.y = 1.06;
  group.add(cap);
  const label = mesh(new BoxGeometry(.22, .28, .018), materials.warning, false, true);
...
export function createBarrier(materials, width = 2.8) {
  ...
  slab.position.y = .36;
  group.add(slab);
  const stripe = mesh(new BoxGeometry(width * .88, .035, .435), materials.warning, false, true);
  stripe.position.set(0, .43, 0);
  stripe.rotation.z = -.08;
  group.add(stripe);
  const cap = mesh(new BoxGeometry(width * .82, .055, .46), materials.trim, false, true);
  cap.position.y = .77;
  group.add(cap);
...
export function createWeaponRack(materials) {
  const group = new Group();
  const frame = mesh(new BoxGeometry(2.2, 1.8, .12), materials.metalDark);
  frame.position.y = .9;
  group.add(frame);
  const shelfGeometry = new BoxGeometry(2.05, .08, .32);
  for (const y of [.32, .82, 1.32]) { const shelf = mesh(shelfGeometry, materials.metal); shelf.position.set(0, y, -.2); group.add(shelf); }
  const gunGeometry = new BoxGeometry(1.35, .09, .1);
  for (let i = 0; i < 3; i += 1) {
...
export function createTargetDummy(materials, id) {
  ...
  group.add(torso);
  const head = mesh(new IcosahedronGeometry(.28, 1), materials.target);
  head.position.y = 2.16;
  head.userData = { target: torso.userData.target, hitZone: 'head' };
  group.add(head);
  const chest = mesh(new CylinderGeometry(.22, .22, .02, 16), materials.targetAccent);
  chest.rotation.x = Math.PI / 2;
  chest.position.set(0, 1.42, -.17);
  chest.userData = torso.userData;
  group.add(chest);
  const armGeometry = new CylinderGeometry(.07, .09, .74, 8);
  ...
  const targetGlow = mesh(new TorusGeometry(.31, .022, 5, 16), materials.cyanHot, false, false);
  targetGlow.rotation.x = Math.PI / 2;
  targetGlow.position.set(0, 1.42, -.2);
  targetGlow.userData.targetGlow = true;
  group.add(targetGlow);
  const pole = mesh(new CylinderGeometry(.07, .07, 1.2, 8), materials.metalDark);
  pole.position.y = .48;
  group.add(pole);
  const base = mesh(new CylinderGeometry(.45, .5, .08, 8), materials.black);
  base.position.y = .06;
  group.add(base);
...
export function createPlanter(materials, foliageDensity = 1) {
  const group = new Group();
  const box = mesh(new BoxGeometry(1.8, .46, .76), materials.concreteDark);
  box.position.y = .23;
  group.add(box);
  const leafMaterial = materials.militaryPaint;
  const count = Math.max(2, Math.round(5 * foliageDensity));
  for (let i = 0; i < count; i += 1) {
    const leaf = mesh(new IcosahedronGeometry(.22 + i * .018, 0), leafMaterial, true, false);
...
export function createPipe(materials, length = 5, vertical = false) {
  const pipe = mesh(new CylinderGeometry(.11, .11, length, 10), materials.metalDark);
...
export function createCable(materials, points) {
  ...
  const cable = new Mesh(new TubeGeometry(curve, Math.max(4, points.length * 3), .035, 5, false), materials.rubber);
...
export function createDoorFrame(materials, width = 1.8, height = 2.7) {
  const group = new Group();
  const postGeometry = new BoxGeometry(.16, height, .22);
  ...
  const header = mesh(new BoxGeometry(width + .16, .16, .22), materials.glassFrame);
