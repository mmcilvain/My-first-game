...
class GameApp {
  constructor() {
    ...
    this.input = { keys: new Set(), movement: new THREE.Vector2(), mobileMovement: new THREE.Vector2(), lookX: 0, lookY: 0, fire: false, aim: false, sprint: false, crouch: false, mobileCrouch: false, jumpPressed: false, reloadPressed: false, isMobile: this.device.mobile };
    this.idleInput = { ...this.input, movement: new THREE.Vector2(), lookX: 0, lookY: 0, jumpPressed: false, reloadPressed: false, fire: false, aim: false, sprint: false, crouch: false };
    this.sound = new SoundEngine();
    this.dom = { canvas: $('#gameCanvas'), loading: $('#loadingScreen'), loadingStage: $('#loadingStage'), loadingBar: $('#loadingBar'), start: $('#startScreen'), hud: $('#hud'), mobile: $('#mobileControls'), pause: $('#pauseMenu'), error: $('#errorScreen'), errorMessage: $('#errorMessage') };
  ...
  async init() {
    try {
      ...
      this.player = new PlayerController(this.camera, this.physics, this.world.spawn, { sensitivity: this.settings.sensitivity, mobileSensitivity: this.settings.mobileSensitivity, invertY: this.settings.invertY, onStep: (kind) => this.sound.step(kind) });
      this.player.reset();
      ...
      this.weapon = new WeaponSystem(this.camera, this.world, this.physics, this.particles, this.sound, { onTargetHit: (target, details) => this.onTargetHit(target, details) });
  ...
  bindUI() {
    ...
    $('#resetButton').addEventListener('click', () => { this.player.reset(); this.resetCombat(); this.showToast('FIELD RESET // ROUND 1'); });
  ...
  start() {
    ...
    this.showToast('LIVE FIRE // MOVE TO ENGAGE');
  }
  ...
  onTargetHit(target, details = {}) {
    const remaining = this.world.targets.filter((item) => item.userData.targetData.health > 0).length;
    this.hitMarkerTimer = details.headshot ? .42 : .25;
    this.score += details.headshot ? 150 : target.health <= 0 ? 100 : 25;
    if (target.health <= 0) { this.combo += 1; this.bestCombo = Math.max(this.bestCombo, this.combo); } else this.combo = Math.max(1, this.combo);
    this.showToast(details.headshot ? 'HEADSHOT // +150' : target.health <= 0 ? `TARGET DOWN // ${remaining} REMAINING` : `HIT CONFIRMED // ${target.health} HP`);
    if (remaining === 0) { $('#targetStatus').textContent = 'FIELD CLEAR'; $('#targetStatus').style.color = '#ffb74d'; this.roundResetTimer = 2.2; }
  }

  resetCombat() {
    this.world.resetTargets(); this.round = 1; this.score = 0; this.combo = 0; this.roundResetTimer = 0; this.hitMarkerTimer = 0;
    if (this.weapon) { this.weapon.ammo = this.weapon.magazineSize; this.weapon.reserve = 120; this.weapon.reloading = false; }
    $('#targetStatus').textContent = 'TARGETS ACTIVE'; $('#targetStatus').style.color = '';
  }
  ...
  frame(time) {
    ...
    this.input.lookX = 0; this.input.lookY = 0; this.input.jumpPressed = false; this.input.reloadPressed = false;
